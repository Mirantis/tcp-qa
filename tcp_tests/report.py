...
def run_cli():
    ...
    cli_process_link.add_argument(
        ...
        default='run',
        help="run - Search marked tests with defects is compared by name "
             "of the run and test in the plan.\n"
             "plan - Search marked tests with defects is compared by name "
             "of test in all runs in the plan.")
    cli_process_link.add_argument(
...
def mark_failed_results(t_cl, marked_res, failed_res, s_cov, t_h):
    ...
    for run, result, test in failed_res:
        for m_run, m_result, m_test in marked_res:
            if s_cov == 'run' and run.name == m_run.name \
             and test.title == m_test.title:
                ...
                ret.append([generate_result(t_cl, test, m_result,
                                            m_test), run.id])
            elif s_cov == 'plan' and test.title == m_test.title:
                LOG.info(" MARKED FOUND: Run:{} test: .. {}-{}"
                         .format(run.id, test.title[-72:],
                                 m_result.defects[0]))
                ret.append([generate_result(t_cl, test, m_result,
...
def mark_fails(**kwargs):
    ...
    runs = get_runs(t_client, t_plan, t_a_run)
    marked_results = get_all_failed_results(t_client, marked_runs,
                                            '2,3,4,5,6,7,8,9')
    failed_results = get_all_failed_results(t_client, runs, '5')  # 5-failed
    """
    "get_all_failed_results" returned result format:
    failed_results = [[run, result, test],
                      [run, result, test],
                      ...]
    """
    tests_to_update = mark_failed_results(t_client, marked_results,
                                          ...
                                          t_host)
    """
    "mark_failed_results" returned result format:
    tests_to_update = [[target_tests_to_update_with_defect, target_run_id],
