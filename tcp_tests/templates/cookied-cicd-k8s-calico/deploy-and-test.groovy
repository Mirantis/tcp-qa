...
evaluate(new File("../shared_pipeline.groovy"))
def shared = new shared_pipeline()
...
node ("${NODE_NAME}") {
    ...
    stage("Clean the environment") {
        shared.clean_environment_workspace()
    }
    ...
    stage("Generate the model") {
        ...
        common.printMsg("Start building job 'custom-cookied-model-generator' with parameters:", "purple")
        common.prettyPrint(parameters)
        build job: 'custom-cookied-model-generator',
