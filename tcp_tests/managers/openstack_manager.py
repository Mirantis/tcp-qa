...
class OpenstackManager(ExecuteCommandsMixin):
    ...
    def install(self, commands):
        ...
        horizon_data = self.get_horizon_data()
        self.__config.openstack.horizon_host = horizon_data['horizon_host']
        self.__config.openstack.horizon_port = horizon_data['horizon_port']
        self.__config.openstack.horizon_user = horizon_data['horizon_user']
        self.__config.openstack.horizon_password = horizon_data['horizon_password']
        self.auth_in_horizon(
            horizon_data['horizon_host'],
            horizon_data['horizon_port'],
            horizon_data['horizon_user'],
            horizon_data['horizon_password'])

    def get_horizon_data(self):
        horizon_data = {}
        tgt = 'I@nginx:server and not cfg*'
        pillar_host = 'nginx:server:site:nginx_ssl_redirect_openstack_web:host:name'
        pillar_port = 'nginx:server:site:nginx_ssl_redirect_openstack_web:host:port'
        address_pillars = self._salt.get_pillar(tgt=tgt,
                                                pillar=pillar_host)
        port_pillars = self._salt.get_pillar(tgt=tgt,
                                             pillar=pillar_port)
        horizon_ip = set([ip
                         for item in address_pillars
                         for node, ip in item.items() if ip])
        horizon_port = set([port for item in port_pillars
                            for node, port in item.items() if port])
        ...
        password = self._salt.get_pillar(tgt=tgt, pillar=pillar_password)
        horizon_user = set([user for item in users for node, user
                            in item.items() if user])
        horizon_password = set([pwd for item in password for node, pwd
                                in item.items() if pwd])

        horizon_data.update({'horizon_host': horizon_ip})
        horizon_data.update({'horizon_port': horizon_port})
        horizon_data.update({'horizon_user': horizon_user})
        horizon_data.update({'horizon_password': horizon_password})
        LOG.debug("Data from pillars {}".format(horizon_data))
    ...
    def auth_in_horizon(self, host, port, user, password):
        ...
        url = "{0}:{1}".format(self.__config.openstack.horizon_host,
