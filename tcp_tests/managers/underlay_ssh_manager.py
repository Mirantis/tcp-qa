...
class UnderlaySSHManager(object):
    ...
    def get_logs(self, artifact_name,
        ...
        for node in minion_nodes:
            with self.remote(node['node_name']) as r_node:
                r_node.check_call(('tar '
                                   '--absolute-names '
                                   '--warning=no-file-changed '
                                   '-czf {t} {d}'.format(
                    t='{0}_log.tar.gz'.format(node['node_name']), d='/var/log')))
        with self.remote(master_node['node_name']) as r:
            for node in minion_nodes:
                ...
                r.check_call("rsync {0}:/var/log/*.tar.gz /var/log/".format(node['node_name']))
            r.check_call(cmd)
