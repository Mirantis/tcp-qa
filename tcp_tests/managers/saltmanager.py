...
class SaltManager(ExecuteCommandsMixin):
    ...
    def create_env_jenkins_cicd(self):
        ...
        domain_name = self.get_single_pillar(
            tgt=self.__host, pillar="_param:cluster_domain")
    ...
    def create_env_k8s(self):
        ...
        ['parameters']['haproxy']['proxy']['listen']['k8s_secure']['binds']
        ['parameters']['kubernetes']['master']['admin']
        domain_name = self.get_single_pillar(
            tgt=self.__host, pillar="_param:cluster_domain")
        LOG.info("Domain: {}".format(domain_name))
        ...
        LOG.info("kube_host: {}".format(kube_host))
        kube_apiserver_port = haproxy_params['port']
        LOG.info("kube_apiserver_port: {}".format(kube_apiserver_port))
        kubernetes_admin_user = k8s_params['username']
        LOG.info("kubernetes_admin_user: {}".format(kubernetes_admin_user))
        kubernetes_admin_password = k8s_params['password']
        LOG.info("kubernetes_admin_password: {}".format(kubernetes_admin_password))
