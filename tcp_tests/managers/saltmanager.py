...
class SaltManager(ExecuteCommandsMixin):
    ...
    def create_env_jenkins_day01(self):
        """Creates utils/env_jenkins_day01 file to source it in pipelines or CLI"""
        ...
        with open(env_jenkins_day01_filename, 'w') as f:
            f.write(
                ...
                .format(host=self.host, port=jenkins_port, user=jenkins_user, password=jenkins_pass)
            )

    def create_env_jenkins_cicd(self):
        """Creates utils/env_jenkins_cicd file to source it in pipelines or CLI"""
        ...
        with open(env_jenkins_cicd_filename, 'w') as f:
            f.write(
                ...
                'echo "export JENKINS_START_TIMEOUT=${{JENKINS_START_TIMEOUT}}  # Timeout waiting for job in queue to start building"\n'
                'echo "export JENKINS_BUILD_TIMEOUT=${{JENKINS_BUILD_TIMEOUT}}  # Timeout waiting for building job to complete"\n'
                .format(host=jenkins_host, port=jenkins_port, user=jenkins_user, password=jenkins_pass)
            )

    def create_env_k8s(self):
        """Creates utils/env_k8s file to source it in pipelines or CLI"""
        ...
        with open(env_k8s_filename, 'w') as f:
            f.write(
                ...
                'echo "export kube_host=${{kube_host}}  # Kube API host"\n'
                'echo "export kube_apiserver_port=${{kube_apiserver_port}}  # Kube API port"\n'
                'echo "export kubernetes_admin_user=${{kubernetes_admin_user}}  # Kube API username"\n'
                'echo "export kubernetes_admin_password=${{kubernetes_admin_password}}  # Kube API password"n\n'
                .format(host=kube_host, port=kube_apiserver_port, user=kubernetes_admin_user, password=kubernetes_admin_password)
            )
