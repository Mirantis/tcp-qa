...
class RuntestManager(object):
    ...
    def prepare(self):
        ...
        barbican_pillar = self.__salt_api.get_pillar(
            tgt="ctl01*",
            pillar=" _param:barbican_integration_enabled")
        control_name = 'ctl01.' + self.domain_name
        LOG.info("control name: {}".format(control_name))
        barbican_integration = bool(barbican_pillar[0].get(control_name))
        LOG.info("barbican_integration: {}".format(str(barbican_integration)))
        commands = [
            {
