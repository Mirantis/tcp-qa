...
class RuntestManager(object):
    ...
    cirros_pillar = 'salt-call --out=newline_values_only pillar.get', \
    ...
    def prepare(self, dpdk=None):
        ...
        commands = [
                        ...
                        salt_call_cmd + " state.sls runtest")},
            {
                ...
                'cmd': ("set -ex;" +
                        "cirros_url=$(" + self.cirros_pillar + ") && " +
                        salt_cmd + self.target_name + " cmd.run " +
                        "wget --directory-prefix /tmp/ " + "$cirros_url")},
        ]
