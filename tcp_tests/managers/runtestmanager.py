...
class RuntestManager(object):
    ...
    def prepare(self, dpdk=None):
        ...
        step = {'cmd': dpdk_pillar_cmd, 'node_name': self.compute_name}
        dpdk_result = self.__salt_api.execute_command(step, 'Check dpdk')
        LOG.info("DPDK result {}".format(dpdk_result['stdout']))
