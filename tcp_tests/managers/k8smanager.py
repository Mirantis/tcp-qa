...
class K8SKubectl:
    ...
    def run(self, name, image, port, replicas=None):
    ...
    def expose(self, resource, resource_name, port,
               service_type='ClusterIP', service_name=None):
        cmd = "kubectl expose {0} {1} --port={2} --type={3}".format(
            resource, resource_name, port, service_type)
...
class K8SSampleDeployment:
    ...
    def expose(self, service_type='ClusterIP'):
        return self._deployment.expose(service_type, self._port)
