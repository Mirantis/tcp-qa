...
# ---------------------------- Virtlet methods -------------------------------
    ...
    def run_virtlet_conformance(self, timeout=60 * 60,
        ...
        cmd = ("docker run --net=host -it {0} /virtlet-e2e-tests "
               "-alsologtostderr -cluster-url http://127.0.0.1:8080 "
               "-ginkgo.focus '\[Conformance\]' "
               "| tee {1}".format(
                self.__config.k8s_deploy.kubernetes_virtlet_image, log_file))
        LOG.info("Executing: {}".format(cmd))
        with self.__underlay.remote(
                node_name=self.ctl_host) as remote:
            result = remote.check_call(cmd, timeout=timeout)['stdout']
            LOG.info("Test results: {}".format(result))
        return result
