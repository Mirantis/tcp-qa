...
class K8SManager(ExecuteCommandsMixin):
    ...
    def _api_init(self):
        ...
        certificate_authority = ca_result['stdout'][0]
        cluster_name = settings.LAB_CONFIG_NAME
        user_name = self.__config.k8s_deploy.kubernetes_admin_user
        password = self.__config.k8s_deploy.kubernetes_admin_password
        api_server = 'https://{0}:{1}'.format(
            self.__config.k8s.kube_host, self.__config.k8s.kube_apiserver_port)
        context_user_name = '{0}-{1}'.format(user_name, cluster_name)
        ...
        self._api_core_client = client.CoreV1Api(api_client)
        self._api_apps_client = client.AppsV1Api(api_client)
        self._api_extensions_client = client.ExtensionsV1beta1Api(api_client)
...
class K8SPod(K8SNamedResource):
    ...
    def create(self, body=None, file_path=None):
        ...
        pod = self._manager.api.create_namespaced_pod(self._namespace, body)
...
class K8SService(K8SNamedResource):
    ...
    def create(self, body=None, file_path=None):
        ...
        svc =\
            self._manager.api.create_namespaced_service(self._namespace, body)
    ...
    def get_ip(self, external=False):
        if external:
            return self.read().status.load_balancer.ingress.ip
        else:
...
class K8SDaemonSet(K8SNamedResource):
    ...
    def create(self, body=None, file_path=None):
        ...
        ds = self._manager.api_apps.create_namespaced_daemon_set(
    ...
    def read(self):
        return self._manager.api_apps.read_namespaced_daemon_set(
...
class K8SDeployment(K8SNamedResource):
    ...
    def create(self, body=None, file_path=None):
        ...
        dep = self._manager.api_apps.create_namespaced_deployment(
            self._namespace, body)
    ...
    def read(self):
        return self._manager.api_apps.read_namespaced_deployment(
            self._name, self._namespace)
...
class K8SNamespace(K8SNamedResource):
    ...
    def read(self):
        return self._manager.api.read_namespace(self._name)

    def patch(self, body):
        self._manager.api.patch_namespace(self._name, body)
...
class K8SNode(K8SNamedResource):
    ...
    def read(self):
        return self._manager.api.read_node(self._name)
