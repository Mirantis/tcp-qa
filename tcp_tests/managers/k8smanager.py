...
class K8SSampleDeployment:
    ...
    def expose(self, service_type='ClusterIP'):
        ...
        namespace = self._deployment.namespace
        self._manager.kubectl.expose(
            namespace, 'deployment', self._deployment.name,
            self._port, service_name, service_type=service_type)
        self._svc = self._manager.api.services.get(
