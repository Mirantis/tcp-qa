...
class K8SManager(ExecuteCommandsMixin):
    ...
    def wait_pod_phase(self, pod_name, phase, namespace=None, timeout=60):
        ...
        LOG.info('K8S USER:: {}'.format(self.__config.k8s_deploy.kubernetes_admin_user))
        LOG.info('PASSWORD:: {}'.format(self.__config.k8s_deploy.kubernetes_admin_password))
        LOG.info('K8S HOST:: {}'.format(self.__config.k8s.kube_host))
        LOG.info('K8S PROXY:: {}'.format(self.get_proxy_api()))
        LOG.info('K8S PORT:: {}'.format(self.__config.k8s.kube_apiserver_port))
...
# ---------------------------- Virtlet methods -------------------------------
    ...
    def start_k8s_cncf_verification(self, timeout=60 * 90):
        fix_rbac = ("kubectl create clusterrolebinding "
                    "root-cluster-admin-binding --clusterrole=cluster-admin "
                    "--user=admin")
        ...
        with self.__underlay.remote(
                node_name=self.ctl_host) as remote:
            # Thus workaround should be deleted asap and clusterrole for
            # admin should be created during deployment
            remote.check_call(fix_rbac)
            ...
            expected = [0, 1]
            helpers.wait(
                lambda: remote.check_call(wait_cmd, expected=expected).
                            exit_code == 0,
