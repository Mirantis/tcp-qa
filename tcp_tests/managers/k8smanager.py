...
# ---------------------------- Virtlet methods -------------------------------
    ...
    def shutdown_node(self, node_name, warm=True, reboot=True, timeout=10*60):
        """
        Shutdown node by name

        :param node_name: str such as "ctl02.k8s-ha-calico.local"
        :param warm: if True, then warm shutdown using "shutdown +1" command
        :param reboot: if True, then start node after shutdown
        :param timeout: int timeout in seconds (doubled for reboot)
        :return:
        """
        if warm:
