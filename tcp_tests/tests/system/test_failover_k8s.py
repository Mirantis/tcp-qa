...
class TestFailoverK8s(object):
    ...
    def test_k8s_master_vip_migration(self, show_step, k8s_deployed, underlay,
        ...
        show_step(4)
        # We cannot shutdown node because checks below use salt grains
        # and they will fail by timeout if node do not become available
        k8s_actions.shutdown_node(minion_vip, warm=False, reboot=True)
