...
class TestFailoverK8s(object):
    ...
    def test_k8s_master_vip_migration(self, show_step, k8s_deployed, underlay,
        ...
        show_step(4)
        underlay.check_call(cmd="shutdown +1", node_name=minion_vip)
        hardware.wait_for_node_state(minion_vip, state='offline', timeout=600)
        hardware.start_node(minion_vip)
        hardware.wait_for_node_state(minion_vip, state='active', timeout=600)
