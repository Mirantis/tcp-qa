...
class TestUpdateMcpCluster(object):
    ...
    def test_update_drivetrain(self, salt_actions, show_step, _):
        ...
        Scenario:
            ...
            2. Run job git-mirror-downstream-mk-pipelines for release/proposed/2019.2.0 (duration <1min)
            4. Run job git-mirror-downstream-pipeline-library for release/proposed/2019.2.0 (duration <1min)
            6. If jobs are passed start 'Deploy - upgrade MCP Drivetrain' job (duration ~30min)
        """
        salt = salt_actions
