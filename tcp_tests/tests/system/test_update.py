...
class TestUpdateMcpCluster(object):
    ...
    def test_update_galera(self, salt_actions, reclass_actions, show_step, _):
        ...
        show_step(2)
        output = salt.enforce_state("I@jenkins:client", "jenkins.client")

        LOG.info(json.dump(output, indent=4))
        ...
        show_step(4)
        output = salt.enforce_state("dbs*", "saltutil.refresh_pillar")
        LOG.info(json.dump(output, indent=4))

        # ############# Add repositories with new Galera packages #######
        show_step(5)
        output = salt.enforce_state("dbs*", "linux.system.repo")
        LOG.info(json.dump(output, indent=4))
        output = salt.enforce_state("cfg*", "salt.master")
        LOG.info(json.dump(output, indent=4))
