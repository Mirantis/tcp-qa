...
class TestUpdatePikeToQueens(object):
    ...
    def test_upgrade_pike_queens(self,
        ...
        list_nodes = underlay_actions.check_call(
            node_name=cfg_node, verbose=verbose,
            cmd="salt-key -l accepted | grep -v Accepted")['stdout_str'].splitlines()
        LOG.info(list_nodes)
        for node in list_nodes:
            salt_pillars = underlay_actions.check_call(
                node_name=cfg_node, verbose=verbose,
                cmd="salt {} config.get orchestration:upgrade:applications --out=json".format(node))
            node_applications = json.loads(salt_pillars['stdout_str'])[node]['__reclass__:applications']
            LOG.info(node_applications)
            for service in services_for_upgrade:
                if service in node_applications:
                    post_upgrade = underlay_actions.check_call(
                        node_name=cfg_node, verbose=verbose,
                        cmd="salt {} state.apply {}.upgrade.post".format(node, service))
        # ######################## Run CPV ##########################
        show_step(8)
        # job_name = 'cvp-sanity'
        # job_parameters = {
