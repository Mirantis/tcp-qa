...
class TestAddressedIssuesMu6(object):
    ...
    def test_gnocchi_measurements(self, show_step,
        ...
        """Apply fix for 'Missing measurements in Gnocchi on environments with
        Barbican'
        ...
        r = salt.get_single_pillar('I@salt:master',
                                  '_param:barbican_enabled')
        LOG.info("{} {} ".format(r, type(r)))
        if salt.get_single_pillar('I@salt:master',
                                  '_param:barbican_enabled') == 'False':
    ...
    def test_stacklight_sf_notifier_sfdc_sandbox_enabled(self, show_step,
            salt_actions, reclass_actions):
