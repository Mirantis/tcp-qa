...
class TestOfflineDeployment(object):
    ...
    def test_deploy_day1(self, show_step, underlay, common_services_deployed,
        ...
        underlay.check_call(
            node_name=cfg_node,
            verbose=verbose,
            cmd="""timeout 300s /bin/bash -c 'while ! salt-call test.ping; do echo "salt master still isnt running"; sleep 10; done'"""
        )  # noqa
        ...
        underlay.check_call(
            ...
            'FTvqwe7ybBp68gPar2:5mcctTAXVL8mns4ef4:zrA9LZwu2tMc8BAZpsPUfwWwTyQnAtDN'
        )  # noqa

        underlay.check_call(
            node_name=cfg_node,
            verbose=verbose,
            cmd="maas mirantis maas set-config name=upstream_dns value='10.10.0.15 8.8.8.8 8.8.4.4'"
        )  # noqa
        ...
        underlay.check_call(
            ...
            "subnet=$(maas mirantis subnets read | jq '.[] | select(.name==\"10.10.0.0/16\") | .id')"
        )  # noqa
        underlay.check_call(
            node_name=cfg_node,
            verbose=verbose,
        ...
        cmd = "salt -C 'I@salt:control or I@nova:compute or I@neutron:gateway' " \
              "cmd.run 'touch /run/is_rebooted'"
        underlay.check_call(node_name=cfg_node, verbose=verbose, cmd=cmd)

        cmd = "salt --async -C 'I@salt:control' cmd.run 'salt-call state.sls " \
              "linux.system.user,openssh,linux.network;reboot'"
        underlay.check_call(node_name=cfg_node, verbose=verbose, cmd=cmd)

        cmd = "salt --async -C 'I@nova:compute' cmd.run 'salt-call state.sls " \
              "linux.system.user,openssh,linux.network;reboot'"
        underlay.check_call(node_name=cfg_node, verbose=verbose, cmd=cmd)

        cmd = "salt --async -C 'I@neutron:gateway' cmd.run 'salt-call state.sls " \
              "linux.system.user,openssh,linux.network;reboot'"
        underlay.check_call(node_name=cfg_node, verbose=verbose, cmd=cmd)

        time.sleep(300)  # TODO: Add ssh waiter
        ...
        cmd = """salt -C 'I@salt:control or I@nova:compute or I@neutron:gateway' """ \
              """cmd.run '[ -f "/run/is_rebooted" ] && echo "Has not been rebooted!" || echo "Rebooted"' """
