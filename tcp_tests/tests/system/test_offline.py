...
class TestOfflineDeployment(object):
    ...
    def test_boot_day1(self, show_step, underlay):
        ...
        show_step(8)
        cmd = """   timeout 7200s bash -c 'hosts=$(maas mirantis nodes read | jq -r ".[] | select(.node_type_name==\\"Machine\\") | select(.status_name==\\"Ready\\") | .hostname "); while ! [ $(echo "$hosts" | wc -w) -eq 10 ]; do echo "Ready hosts:\n$hosts"; sleep 30; hosts=$(maas mirantis nodes read | jq -r ".[] | select(.node_type_name==\\"Machine\\") | select(.status_name==\\"Ready\\") | .hostname "); done '   """ # noqa
        ...
        jenkins = JenkinsClient(
            ...
            username='admin',
            password='r00tme')
        build = jenkins.run_build('deploy_openstack')
        ...
        assert (
            jenkins.build_info(name=build[0],
                               build_id=build[1])['result'] == 'SUCCESS',
