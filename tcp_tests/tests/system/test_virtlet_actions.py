...
class TestVirtletActions(object):
    ...
    def __init__(self):
        self.data_dir = os.path.join(os.path.dirname(__file__), 'testdata/k8s')
        super(TestVirtletActions, self).__init__()
    ...
    def test_virtlet_create_delete_vm(self, show_step, config, k8s_deployed):
        ...
        vm_pod = k8s_deployed.api.pods.create(
            file_path=os.path.join(self.data_dir, 'cirros-vm.yaml'))
        show_step(2)
    ...
    def test_vm_resource_quotas(self, show_step, config, k8s_deployed):
        ...
        pod_body = read_yaml_body(
            file_path=os.path.join(self.data_dir, 'cirros-vm.yaml'))
        pod_body['metadata']['annotations']['VirtletVCPUCount'] = cpu
        ...
        assert stats['vcpu.current'] == cpu
        assert stats['balloon.maximum']/1024 == memory_mb
