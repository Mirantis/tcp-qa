...
class TestOpenContrail(object):
    ...
    def test_opencontrail(self, config, openstack_deployed,
                          show_step, sl_deployed, openstack_actions):
        ...
        if settings.RUN_TEMPEST:
            openstack_actions.run_tempest(target='ctl01', pattern=settings.PATTERN)
            openstack_actions.download_tempest_report(stored_node='ctl01')
        LOG.info("*************** DONE **************")
