...
class TestBackupRestoreCassandra(object):
    ...
    @pytest.mark.parametrize("_", [settings.ENV_NAME])
    @pytest.mark.run_mcp_update
    def test_backup_restore_cassandra_manual(self, salt_actions,
                                             reclass_actions,
                                             show_step, underlay_actions, _):
        ...
        for node_name, statuses_output in statuses[0]["return"][0].iteritems():
            for status_line in statuses_output.splitlines():
                if not status_line.startswith("=="):
                    ...
                    status = status.strip()
                    assert status in ["active", "backup"], \
                        "On node {} service {} has unexpected status after " \
                        "restore: {}".format(node_name, service.strip(), status)
