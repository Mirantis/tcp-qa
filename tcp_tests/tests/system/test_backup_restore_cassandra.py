...
class TestBackupRestoreCassandra(object):
    ...
    def verify_network_exists(self, underlay_actions, network_name):
        networks = underlay_actions.check_call(
            "source /root/keystonercv3 && openstack network list --name {}".format(network_name),
            node_name=self.cfg_node,
    ...
    def test_backup_restore_cassandra(self, salt_actions, reclass_actions,
        ...
        self.create_network(underlay_actions, "test1")
        ...
        #     description, "\n".join(stages))
        # self.verify_network_exists(underlay_actions, "backuptest2")
    ...
    def test_backup_restore_cassandra_manual(self, salt_actions, reclass_actions,
        ...
        reclass = reclass_actions
        self.prepare_and_create_backup(salt, show_step)
        self.create_network(underlay_actions, "backuptest2")
        self.add_restore_params(show_step, reclass)
        show_step(10)
        ...
        for node_name, statuses_output in statuses[0]["return"][0].iteritems():
            ...
            for status_line in statuses_output.splitlines():
                if not status_line.startwith("=="):
                    assert status_line.endwith("active"), "{} is not OK".format(status_line)
