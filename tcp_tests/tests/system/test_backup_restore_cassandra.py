...
class TestBackupRestoreCassandra(object):
    ...
    @pytest.fixture()
    def handle_restore_params(self, reclass):
        reclass.add_key(
    ...
    # def test_backup_creation(self, salt_actions, reclass_actions,
    #                          drivetrain_actions,
    ...
    #     LOG.info(backup_on_client_node)
    #     assert len(backup_on_client_node['return'][0][0].values()) > 0, \
    ...
    #     assert len(backup_on_server_node['return'][0][0].values()) > 0, \
    ...
    def test_restore_automatic(self, salt_actions, reclass_actions,
        ...
        Scenario:
            ...
            9. Verify that a complete backup has been created
            10. Restore from the backup. Prepare parameters
            11. Restore from the backup. Add job class for restore Cassandra
            12. Restore from the backup. Run Jenkins job
        """
        salt = salt_actions
        reclass = reclass_actions
        dt = drivetrain_actions
    ...
    #         8. Create an instant backup
    #         9. Verify that a complete backup has been created
    #         10. Restore: Prepare parameters
    #         11. Restore from the backup. Stop the supervisor-database service
    #             on the OpenContrail control nodes
    #         12. Restore: Remove the Cassandra files on control nodes
    #         13. Restore: Start the supervisor-database service on the
    #             Cassandra client backup node
    #         14. Restore: Apply the cassandra state
    #         15. Restore: Reboot the Cassandra backup client role node
    #         16. Restore: Reboot the other OpenContrail control nodes
    #         17. Restore: Restart the supervisor-database service
    #         18. Restore: verify that OpenContrail is in correct state
    #     """
    #     salt = salt_actions
    #     reclass = reclass_actions
    ...
    #         "cmd.run",
    #         "ls /var/backups/cassandra/full")
    #     assert len(backup_on_client_node['return'][0].values()) > 0, \
    #         "Backup is not created on Cassandra client node"
    #     backup_on_server_node = salt.run_state(
    #         "I@cassandra:backup:server",
    #         "cmd.run",
    #         "ls /srv/volumes/backup/cassandra/full")
    #     assert len(backup_on_server_node['return'][0].values()) > 0, \
    #         "Backup is not created on Cassandra server node"
    #     show_step(10)
    #     self.add_restore_params(reclass)
    #     show_step(11)
