...
class TestBackupRestoreCassandra(object):
    ...
    def get_cfg_fqn(self, salt):
        keystone_client = salt.local("I@keystone:client", "test.ping")
        LOG.info(keystone_client)
        LOG.info(keystone_client['return'][0].keys()[0])
...
#         LOG.info(backup)
    ...
    def test_backup_restore_manual(self, salt_actions,
        ...
        salt = salt_actions
        reclass = reclass_actions
        self.get_cfg_fqn(salt)
        self.remove_restore_params(reclass)

        fixture_network_name = "backuptest2"
        test = underlay_actions.check_call(
            "source /root/keystonercv3 && "
            "openstack network list --name {}".format(fixture_network_name),
            node_name=self.cfg_node,
            raise_on_err=False)["stdout"]
        LOG.info(test)
        LOG.info(test == '')
