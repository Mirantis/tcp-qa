...
class TestBackupRestoreCassandra(object):
    ...
    def test_backup_restore_cassandra(self, salt_actions, reclass_actions,
        ...
        show_step(1)
        reclass.add_bool_key("parameters.cassandra.server.enabled",
                             "True",
                             "cluster/*/infra/backup/server.yml")
        reclass.add_key("parameters.cassandra.server.hours_before_full",
                        "24",
                        "cluster/*/infra/backup/server.yml")
        reclass.add_key(
            "parameters.cassandra.server.full_backups_to_keep",
            "5",
            "cluster/*/infra/backup/server.yml")
        reclass.add_bool_key("parameters.cassandra.server.cron",
                             "True",
                             "cluster/*/infra/backup/server.yml")
        # there is another backup_dir, but not under server
        reclass.add_key("parameters.cassandra.server.backup_dir",
                        "/srv/volumes/backup/cassandra",
                        "cluster/*/infra/backup/server.yml")
        show_step(2)
        reclass.add_bool_key("parameters.cassandra.backup.cron",
                             "True",
        ...
        show_step(3)
        test = salt.run_state("*", "saltutil.refresh_pillar")
        LOG.info(test)
        LOG.info('!!!!!!!!!!!!!!!!')
        show_step(4)
        salt.run_state(
