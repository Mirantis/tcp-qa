...
class TestFailoverCeph(object):
    ...
    def get_ceph_health(self, ssh, node_names):
        ...
        Args:
            ssh (UnderlaySSHManager): tcp-qa SSH manager
            node_names (list): FQDNs of all Ceph OSD nodes

        Returns:
            dict: Ceph health status from each OSD node (output of 'ceph -s'
    ...
    def test_restart_osd_node(
        ...
        # status = all(["OK" in status for node, status in ceph_health.items()])
        ...
        show_step(5)
        jenkins_creds = salt.login_cluster_jenkins()
