...
class TestOpenContrail(object):
    ...
    def test_contrail4_k8s_pipeline_deploy(self, show_step, underlay,
        ...
        # TEST TEST TEST
        test_tgt = 'I@salt:master'
        test_res = salt_deployed.get_pillar(
            test_tgt, 'salt:master:worker_threads')
        LOG.info("RES - {}".format(test_res))
        LOG.info("RES_TYPE - {}".format(type(test_res)))
        show_step(4)
        ...
        # TEST TEST TEST TEST
        LOG.info("jenkins_user: {}, jenkins_passwd: {}, jenkins_host: {}, jenkins_port: {} ".format(jenkins_user,jenkins_passwd,jenkins_host,jenkins_port))
        jenkins = JenkinsClient(
