...
class TestMCPK8sActions(object):
    ...
    def test_k8s_ingress_nginx(self, show_step, config,
        ...
        """
        show_step(1)
        ctl = k8s_deployed.controller_name

        result = salt_deployed.get_pillar(
            tgt=ctl, pillar='kubernetes:common:addons:metallb:enabled')
        metallb = result[0].get(ctl, False)
        LOG.info("{} kubernetes:common:addons:metallb:enabled: {}\n{}"
                 .format(ctl, bool(metallb), result))

        result = salt_deployed.get_pillar(
            tgt=ctl, pillar='kubernetes:common:addons:ingress-nginx:enabled')
        ingress_nginx = result[0].get(ctl, False)
        LOG.info("{} kubernetes:common:addons:ingress-nginx:enabled: {}\n{}"
                 .format(ctl, bool(ingress_nginx), result))
