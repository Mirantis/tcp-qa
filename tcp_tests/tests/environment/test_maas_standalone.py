...
class TestMaasStandalone(object):
    ...
    def test_maas_bm_provision(self, show_step, hardware, underlay,
        ...
        ssh_test_key = day1_underlay.config.underlay.ssh_keys[0]['public']
        verbose = True
        ...
        r, f = day1_underlay.salt.enforce_state('cfg01*', 'maas.machines')
        LOG.info(r)
        LOG.info(f)
        ...
        result = \
            day1_underlay.salt.get_pillar(cfg_node,
                                          '_param:jenkins_salt_api_url')
        result = result[0].get(cfg_node)

        jenkins = JenkinsClient(
            host='http://{host}:8081'.format(
                host=day1_underlay.config.salt.salt_master_host),
            username='admin',
            password='r00tme')
        ...
        openstack = managers.openstack_manager.OpenstackManager(
           day1_underlay.config, underlay, hardware,
           day1_underlay.salt)
