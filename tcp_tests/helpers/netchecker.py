...
class Netchecker(object):
    ...
    def calico_allow_netchecker_connections(self):
        srv_pod_ip = self.get_netchecker_pod().read().status.host_ip
        ...
        self._api.networkpolicies.get(
            namespace=self._namespace).create(body=body)
    ...
    def kubernetes_allow_traffic_from_agents(self):
        ...
        self._api.networkpolicies.get(
            namespace=self._namespace).create(body=kubernetes_policy)
        self._api.networkpolicies.get(
            namespace=self._namespace).create(body=kubernetes_policy_hostnet)

    @utils.retry(3, requests.exceptions.RequestException)
    def get_metric(self, netchecker_pod_port):
        kube_host_ip = self.get_srv_pod().read().status.host_ip
