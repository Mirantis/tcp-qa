...
class Netchecker(object):
    ...
    def get_netchecker_pod_ip(self, prefix='netchecker-server-'):
    ...
    def check_network(self, works=True):
        if works:
            assert self.get_connectivity_status().status_code in (200, 204)
        else:
            assert self.get_connectivity_status().status_code == 400

    def wait_check_network(self, works=True, timeout=60, interval=10):
        helpers.wait_pass(
            lambda: self.check_network(self.get_service_port(), works=works),
            timeout=timeout,
            interval=interval)
