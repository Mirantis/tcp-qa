        ...
        for container in ds['spec']['template']['spec']['containers']:
            if container['name'] == 'netchecker-agent':
                ...
                for env_var in container['env']:
                    if env_var['name'] != 'SERVICE_FQDN':
                        continue
                    env_var['value'] = ('netchecker-service.{0}.svc.cluster.'
                                        'local'.format(service_namespace))
...
def get_status(k8sclient, netchecker_pod_port=NETCHECKER_NODE_PORT,
    ...
    netchecker_srv_pod = k8sclient.pods.get(name=pod_name, namespace=namespace)
...
def wait_running(k8sclient, namespace='default', timeout=120, interval=5):
    helpers.wait_pass(
        lambda: get_status(k8sclient, namespace=namespace),
...
def calico_allow_netchecker_connections(underlay, kube_host_ip, namespace):
