...
def k8s_copy_sample_testdata(config, underlay, k8s_deployed):
    ...
    LOG.info("Uploading k8s '{}' directory on controllers".format(directory_to_upload))
    with underlay.remote(host=config.salt.salt_master_host) as r:
        r.upload(os.path.join(os.getcwd(), directory_to_upload),
                 os.path.join('/root/', directory_to_upload))
