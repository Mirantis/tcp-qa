...
def run_cmd(cmd, returnStdout=false, exeption_with_logs=false) {
    def common = new com.mirantis.mk.Common()
    common.printMsg("Run shell command:\n" + cmd, "blue")
    def VENV_PATH='/home/jenkins/fuel-devops30'
    def stdout_log = 'stdout.log'
    def stderr_log = 'stderr.log'
    script = """\
        set +x;
        echo 'activate python virtualenv ${VENV_PATH}';
        . ${VENV_PATH}/bin/activate;
        bash -c 'set -ex; set -ex; ${cmd.stripIndent()}' 1>${stdout_log} 2>${stderr_log}
    """
    def result
    try {
        result = sh(script: script)
        if (returnStdout) {
            def stdout = readFile("$WORKSPACE/${stdout_log}")
            return stdout
        } else {
            return result
        }
        ...
        if (exeption_with_logs) {
            def stdout = readFile("$WORKSPACE/${stdout_log}")
            def stderr = readFile("$WORKSPACE/${stderr_log}")
...
def run_cmd_stdout(cmd) {
    return run_cmd(cmd, returnStdout: true, exeption_with_logs: true)
}
...
def prepare_working_dir() {
        ...
        """)
        //run_cmd(cmd: cmd, exeption_with_logs: true)
}
...
def swarm_bootstrap_salt_cluster_devops() {
        ...
        build_job('swarm-bootstrap-salt-cluster-devops', parameters)
}
...
def swarm_deploy_cicd(String stack_to_install='core,cicd') {
        ...
        build_job('swarm-deploy-cicd', parameters)
}
...
def swarm_deploy_platform(String stack_to_install) {
        ...
        build_job('swarm-deploy-platform', parameters)
}
...
def generate_configdrive_iso() {
        ...
        build_job('create-cfg-config-drive', parameters)
}
