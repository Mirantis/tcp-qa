 ...
 *   INSTALLED_COMPONENTS          Installed components using Jenkins on CICD cluster: "salt,core,cicd,openstack:3200,stacklight:2400",
                                   where 3200 and 2400 are timeouts
...
node ("${PARENT_NODE_NAME}") {
    ...
    dir("${PARENT_WORKSPACE}") {
        try {
            ...
            stage("Run tests") {
                def sources = "export ENV_NAME=${ENV_NAME}\n. ./tcp_tests/utils/env_salt\n"
                if (INSTALLED_COMPONENTS.contains('k8s')) {
                    sources += ". ./tcp_tests/utils/env_k8s\n"
                }
                if (INSTALLED_COMPONENTS.contains('openstack')) {
                    sources += "# TODO: . ./tcp_tests/utils/env_keystonercv3\n"
                }
                def installed = INSTALLED_COMPONENTS.collect {"export ${it}_installed=true\n"}
