        ...
        try {
            ...
            stage("Prepare Packer") {
                ...
                if (fileExists(packer_zipname)) {
                    sh(script: "md5sum -c --status <(echo ${PACKER_ZIP_MD5} ${packer_zipname}) || rm -f ${packer_zipname}", returnStdout: true)
                }
                // If the file is missing or removed, then download it and check the checksum
                if (!fileExists(packer_zipname)) {
                    sh(script: "wget --quiet -O ${zipfile} ${job_env.PACKER_URL}", returnStdout: true)
                    // Should fail the job if not match
                    sh(script: "md5sum -c --status <(echo ${PACKER_ZIP_MD5} ${packer_zipname})", returnStdout: true)
