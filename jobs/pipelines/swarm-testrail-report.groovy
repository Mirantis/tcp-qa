...
node ("${PARENT_NODE_NAME}") {
    ...
    dir("${PARENT_WORKSPACE}") {
        try {
            ...
            def deployment_report_name = shared.run_cmd_stdout("find ${PARENT_WORKSPACE} -name \"deployment_${ENV_NAME}.xml\"")
            def tcpqa_report_name = shared.run_cmd_stdout("find ${PARENT_WORKSPACE} -name \"nosetests.xml\"")
            def tempest_report_name = shared.run_cmd_stdout("find ${PARENT_WORKSPACE} -name \"report_*.xml\"")
//            def k8s_conformance_report_name = shared.run_cmd_stdout("find ${PARENT_WORKSPACE} -name \"report_*.xml\"")
            def stacklight_report_name = shared.run_cmd_stdout("find ${PARENT_WORKSPACE} -name \"report_*.xml\"")
