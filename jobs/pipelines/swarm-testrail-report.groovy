    ...
    dir("${PARENT_WORKSPACE}") {
        try {
            ...
            stage("Deployment report") {
                def report_name = "\$(pwd)/deployment_${ENV_NAME}"
                def testSuiteName = "[MCP] Integration automation"
                def methodname = '{methodname}'
                def testrail_name_template = '{title}'
            ...
            if ('openstack' in stacks) {
                stage("Tempest report") {
                    def report_name = "report_*.xml"
                    def testSuiteName = "[MCP1.1_PIKE]Tempest"
            ...
            if ('stacklight' in stacks) {
                stage("stacklight-pytest report") {
                    def report_name = "report.xml"
                    def testSuiteName = "LMA2.0_Automated"
                    def methodname = "{methodname}"
                    def testrail_name_template = "{title}"
                    shared.upload_results_to_testrail(report_name, testSuiteName, methodname, testrail_name_template)
                }
